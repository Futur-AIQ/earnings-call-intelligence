...
@lru_cache(maxsize=10)
def load_full_transcript(run_id: str) -> str:
    """Load the full transcript text for a run. Cached per run_id.

    Tries pipeline_output.json first (has raw_text), falls back to
    stage_extraction_result.json (join page texts).
    Returns empty string if no transcript found.
    """
    run_dir = RUNS_DIR / run_id
    if not run_dir.exists():
        logger.warning(f"Run directory not found for transcript: {run_id}")
        return ""

    # Source 1: pipeline_output.json -> raw_text
    pipeline_file = run_dir / "pipeline_output.json"
    if pipeline_file.exists():
        try:
            with open(pipeline_file, "r", encoding="utf-8") as f:
                pipeline_data = json.load(f)
            if "raw_text" in pipeline_data and pipeline_data["raw_text"]:
                text = pipeline_data["raw_text"]
                logger.info(f"Loaded transcript for {run_id}: {len(text):,} chars from pipeline_output.json")
                return text
        except (json.JSONDecodeError, IOError) as e:
            logger.warning(f"Failed to read pipeline_output.json: {e}")

    # Source 2: stage_extraction_result.json -> join pages
    extraction_file = run_dir / "stage_extraction_result.json"
    if extraction_file.exists():
        try:
            ...
            if pages:
                ...
                logger.info(f"Loaded transcript for {run_id}: {len(text):,} chars from extraction pages")
                return text
            if "raw_text" in extraction_data:
                text = extraction_data["raw_text"]
                logger.info(f"Loaded transcript for {run_id}: {len(text):,} chars from extraction raw_text")
                return text
        except (json.JSONDecodeError, IOError) as e:
            logger.warning(f"Failed to read stage_extraction_result.json: {e}")

    logger.warning(f"No transcript text found for run {run_id}")
    return ""
...
def invalidate_cache(run_id: Optional[str] = None):
    ...
    load_full_transcript.cache_clear()
