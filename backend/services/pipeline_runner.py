...
async def _run_actual_pipeline(
    ...
    # Initialise all expected stages as "pending" so the stepper shows up immediately
    stage_names = ["extraction", "metadata", "boundary", "speakers", "qa", "strategic"]
    for sn in stage_names:
        await _update_stage_status(run_id, sn, "pending")

    # Thread-safe callback: called from sync pipeline thread, posts updates to the async loop
    loop = asyncio.get_running_loop()

    def sync_stage_callback(stage_name: str, status: str) -> None:
        fut = asyncio.run_coroutine_threadsafe(
            _update_stage_status(run_id, stage_name, status), loop
        )
        try:
            fut.result(timeout=10)
        except Exception:
            pass  # Never fail the pipeline over a status update

    # Run pipeline in executor (sync → async bridge)
    try:
        result = await loop.run_in_executor(
            None,
            lambda: run_pipeline_v2(
                str(file_path),
