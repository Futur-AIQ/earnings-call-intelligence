...
def _ascii_safe(text: str) -> str:
    """Strip diacritics and replace remaining non-ASCII chars.

    NFKD decomposes accented letters (e.g. Ö → O + combining diaeresis),
    then encoding to ASCII with errors='ignore' drops the combining marks,
    leaving plain ASCII equivalents (Ö→O, ä→a, ü→u, –→?, etc.).
    Non-decomposable chars (em-dash, curly quotes) are dropped silently.
    This prevents gpt-oss:20b from returning empty on prompts with Unicode.
    """
...
def _invoke_llm(prompt: str, max_tokens: int = 200, require_json: bool = False) -> str:
    """Invoke LLM with a prompt and return raw response.

    Strategy:
    1. Try primary model (gpt-oss:20b) up to 3x, escalating temperature each
       retry to break deterministic EOS-token loops that some models exhibit on
       specific prompt content.
    2. If all primary attempts fail, try the fallback model (gemma3:latest) once
       at temperature=0.0 — it does not have the EOS-loop issue.
    """
    # Temperature schedule: 0.0 → 0.2 → 0.5 on primary retries.
    temperatures = [0.0, 0.2, 0.5]

    def _is_valid(response: str) -> bool:
        """Check if response passes require_json validation (or any non-empty when not required)."""
        if not response:
            return False
        if not require_json:
            return True
    ...
    for attempt in range(3):
        ...
        llm = create_json_llm_client(num_predict=max_tokens, temperature=temp)
        try:
            raw = llm.invoke(prompt)
            response = raw.content if hasattr(raw, "content") else raw
            response = response.strip() if response else ""
            if _is_valid(response):
                return response
            if not response:
                logger.warning("llm_empty_response", attempt=attempt + 1, max_attempts=3, temperature=temp)
            else:
                json_match = re.search(r'\{[\s\S]*\}', response)
                if json_match:
                    try:
                        ...
                        logger.warning("llm_invalid_json", attempt=attempt + 1, chars=len(response), error=str(je)[:80])
                else:
                    logger.warning("llm_no_json_found", attempt=attempt + 1, chars=len(response))
        except Exception as e:
            logger.error("llm_invocation_failed", error=str(e), attempt=attempt + 1)
        if attempt < 2:
            time.sleep(1)

    # All primary model attempts failed — try fallback model once
    logger.warning("llm_primary_failed_trying_fallback", max_tokens=max_tokens)
    try:
        fallback_llm = create_json_llm_client(use_fallback=True, num_predict=max_tokens, temperature=0.0)
        raw = fallback_llm.invoke(prompt)
        response = raw.content if hasattr(raw, "content") else raw
        response = response.strip() if response else ""
        # Bug 2 fix: apply same _is_valid() check as primary attempts
        if _is_valid(response):
            logger.info("llm_fallback_succeeded", chars=len(response))
            return response
        if not response:
            logger.warning("llm_fallback_empty")
        else:
            logger.warning("llm_fallback_invalid_json", chars=len(response))
    except Exception as e:
...
def verify_speaker_registry_with_context(
    ...
    tentative_roles: Optional[dict] = None,
    ...
    """
    # Sanitize inputs to ASCII — gpt-oss:20b returns empty on non-ASCII prompts
    opening_remarks_text = _ascii_safe(opening_remarks_text)
    qa_session_text = _ascii_safe(qa_session_text)

    # Build candidate summary for prompt — cap at 25 candidates, 100 chars per turn
    # to keep total prompt under ~10k chars (gpt-oss:20b loops on empty tokens past ~11.5k)
    candidate_summary = ""
    for i, cand in enumerate(speaker_candidates[:25]):
        raw_name = cand.get("name", "Unknown")
        name = _ascii_safe(raw_name)
        turns = cand.get("turns", [])
        turn_samples = "\n    ".join([_ascii_safe(t)[:100] + "..." if len(t) > 100 else _ascii_safe(t) for t in turns[:2]])
        # Include section-based tentative role and company as hints to reduce LLM guesswork
        tentative_hint = ""
        if tentative_roles:
            t_data = tentative_roles.get(raw_name) or tentative_roles.get(name)
            if t_data:
                t_role, _, t_company = t_data
                tentative_hint = f"\n  Tentative role (from section analysis): {t_role}"
                if t_company:
                    tentative_hint += f", company: {_ascii_safe(t_company)}"
        candidate_summary += f"""
CANDIDATE {i+1}: "{name}"{tentative_hint}
  Sample turns:
    {turn_samples}
"""
...
- Aliases must be distinct textual variants that appear in the document. Canonical name must NOT appear in its own alias list.

IMPORTANT: Output ONLY valid JSON. No explanations before or after."""

    response = _invoke_llm(prompt, max_tokens=4096, require_json=True)
...
def _parse_speaker_registry_decision(
    ...
    try:
        ...
        if json_match:
            ...
            # Bug 1 fix: reconcile silently dropped candidates.
            # Every original candidate must appear in either verified_speakers or
            # rejected_candidates. If the LLM silently omitted one, restore it as unknown.
            rejected_candidates = data.get("rejected_candidates", [])
            accounted_names: set[str] = set()
            for v in verified:
                accounted_names.add(v.canonical_name.lower())
                for alias in v.aliases:
                    accounted_names.add(alias.lower())
            for r in rejected_candidates:
                accounted_names.add(r.get("name", "").lower())

            for cand in original_candidates:
                cand_name = cand.get("name", "Unknown")
                if cand_name.lower() not in accounted_names:
                    logger.warning("candidate_silently_dropped", name=cand_name[:50])
                    verified.append(VerifiedSpeaker(
                        canonical_name=cand_name[:50],
                        role="unknown",
                        title=None,
                        company=None,
                        aliases=[],
                        justification="Silently dropped by LLM - restored with unknown role",
                    ))
    ...
    verified = []
    for cand in original_candidates[:25]:
...
# =============================================================================
# Moderator Intro Line Extraction (Analyst Discovery)
# =============================================================================

def _parse_analyst_list_response(response: str) -> list[dict]:
    """Parse LLM JSON array into list of {name, company} dicts."""
    try:
        ...
        for item in data:
            if not isinstance(item, dict):
                continue
            name = (item.get("name") or "").strip()[:50]
            if not name:
                continue
            company = item.get("company")
            if company:
                company = str(company).strip()[:50] or None
            result.append({"name": name, "company": company})
        return result
    except (json.JSONDecodeError, TypeError):
        return []


def extract_analysts_from_moderator_lines(intro_lines: list[str]) -> list[dict]:
    """Extract analyst names and companies from moderator introduction lines.

    Small, focused LLM call that normalizes raw moderator introduction lines
    into structured {name, company} pairs. Handles edge cases like
    "Sandeep Tulsiyan You may go ahead" where there is no explicit company.

    Args:
        intro_lines: Raw moderator introduction lines from the transcript.

    Returns:
        List of {name: str, company: str|None} dicts. Empty list on failure.
    """
    if not intro_lines:
        return []

    # Format lines as numbered list, sanitize for the model
    formatted = "\n".join(
        f'[{i+1}] "{_ascii_safe(line.strip())}"'
        for i, line in enumerate(intro_lines[:20])
    )

    prompt = f"""Extract investor/analyst names and company affiliations from these moderator introduction lines.

{formatted}

Rules:
- Each line introduces an analyst asking a question
- Extract: full name (FirstName LastName), company if mentioned
- Normalize: remove titles (Mr./Ms./Dr.), trim extra words
- If no company: set company to null

Return ONLY valid JSON array:
[{{"name": "Full Name", "company": "Company Name or null"}}]"""

    response = _invoke_llm(prompt, max_tokens=600, require_json=False)
    if not response:
        return []
    return _parse_analyst_list_response(response)


# =============================================================================
# Final Name Validation (Remove Non-Person Entries)
# =============================================================================

def _parse_name_validation_response(response: str) -> dict:
    """Parse LLM {valid, invalid} JSON response for name validation."""
    try:
        json_match = re.search(r'\{[\s\S]*\}', response)
        if not json_match:
            return {}
        ...
        if not isinstance(data, dict):
            return {}
        valid = [str(n).strip() for n in data.get("valid", []) if n]
        invalid = [str(n).strip() for n in data.get("invalid", []) if n]
        return {"valid": valid, "invalid": invalid}
    except (json.JSONDecodeError, TypeError):
        return {}


def validate_final_speaker_names(names: list[str]) -> dict:
    """Validate that canonical speaker names are real people.

    Catches junk that survived Phase A hard rules: job titles without
    a person name, city names, month names, document artifact codes.

    Args:
        names: List of canonical speaker names to validate.

    Returns:
        Dict with {valid: [...], invalid: [...]} or empty dict on failure.
        On empty/failed response, caller should skip removal (fail-safe).
    """
    if not names:
        return {}

    name_list = "\n".join(f"- {_ascii_safe(n)}" for n in names[:30])

    prompt = f"""Which of these are real person names?

{name_list}

Rules:
- VALID: proper human names (FirstName LastName)
- VALID: "Moderator" (role label, always keep)
- INVALID: job titles without a name (Compliance Officer, Company Secretary)
- INVALID: places/cities (Mumbai, London)
- INVALID: months/dates (September, Monday)
- INVALID: document codes (Scrip Code, CIN, ISIN)

Return ONLY valid JSON:
{{"valid": ["Name1", "Name2"], "invalid": ["BadName1"]}}"""

    response = _invoke_llm(prompt, max_tokens=400, require_json=False)
    if not response:
        return {}
    return _parse_name_validation_response(response)
