...
# =============================================================================
# Name token: 1-4 STRICTLY title-case words (each word must start with uppercase A-Z)
# Using a raw string that avoids the (?i) flag bleeding into [A-Z] character classes.
# Each pattern below uses (?i:...) ONLY for the boilerplate text, NOT for the name capture.
_NAME_TOKEN = r"(?:Mr\.\s*|Ms\.\s*|Mrs\.\s*|Dr\.\s*)?([A-Z][A-Za-z\.\-']+(?:\s+[A-Z][A-Za-z\.\-']+){0,3})"
_COMPANY_TOKEN = r"([A-Za-z][A-Za-z\s&,\.]{2,40}?)"

QA_BLOCK_START_PATTERNS = [
    # "The first/next/following/follow-up question is from [the line of] Name [from Company]"
    # (?i:...) applies case-insensitivity ONLY to the boilerplate text, not to the name token.
    # This prevents lowercase phrases ("an individual investor") from matching [A-Z].
    (r"(?i:(?:the\s+)?(?:first|next|following|follow[\-\s]up)\s+question\s+(?:is\s+)?(?:from|comes\s+from)\s+(?:the\s+line\s+of\s+)?)" + _NAME_TOKEN + r"(?:\s+from\s+" + _COMPANY_TOKEN + r")?(?:\s*\.|,|\s|$)", "question_from_line"),
    # "The next is a follow-up question from Name" / "next question is from Name"
    (r"(?i:(?:the\s+)?next\s+(?:is\s+a\s+)?(?:follow[\-\s]up\s+)?question\s+(?:is\s+)?from\s+)" + _NAME_TOKEN + r"(?:\s+from\s+" + _COMPANY_TOKEN + r")?(?:\s*\.|,|\s|$)", "next_question_from"),
    # "We have a question from John Smith from ICICI Securities"
    (r"(?i:we\s+(?:have|got)\s+(?:a\s+)?question\s+from\s+)" + _NAME_TOKEN + r"(?:\s+(?:from|of|with)\s+" + _COMPANY_TOKEN + r")?(?:\s*\.|,|\s|$)", "we_have_question"),
    # "Please go ahead, John" — triggered by explicit name after go-ahead phrase
    (r"(?i:please\s+go\s+ahead\s*[,\s])" + _NAME_TOKEN + r"(?:\s*\.|$)", "go_ahead_name"),
    # "John Smith, you may ask your question" — name at start of line/sentence
    (r"(?:^|(?<=\.\s)|(?<=\n))" + _NAME_TOKEN + r"(?i:[,\s]+(?:you\s+may|please)\s+(?:ask|go\s+ahead|proceed))", "name_may_ask"),
]

# Honorific-only tokens — if the captured name is only one of these, the match is invalid
_HONORIFICS_ONLY = {'mr', 'ms', 'mrs', 'dr', 'prof', 'sir', 'madam', 'ma'}

# Generic phrases that are never valid questioner names (lowercase check)
_INVALID_NAME_PHRASES = {
    'an individual investor', 'a participant', 'an analyst', 'the questioner',
    'individual investor', 'participant', 'our next questioner',
}


# Company/org suffixes that indicate a name is a company, not a person
_COMPANY_SUFFIXES = {
    'investments', 'securities', 'capital', 'asset', 'management', 'fund',
    'bank', 'financial', 'financials', 'advisors', 'partners', 'group',
    'limited', 'ltd', 'inc', 'corp', 'llc', 'llp', 'plc', 'pvt',
    'research', 'brokerage', 'wealth', 'equity', 'equities',
}


def _is_valid_questioner_name(name: str) -> bool:
    """Return True only if the captured text looks like a real person's name."""
    if not name:
        return False
    # Must start with uppercase letter
    if not name[0].isupper():
        return False
    # Must not be too long (real names < 60 chars)
    if len(name) > 60:
        return False
    # Reject if it's just an honorific
    stripped = name.rstrip('.').strip().lower()
    if stripped in _HONORIFICS_ONLY:
        return False
    # Reject known generic phrases
    if name.lower() in _INVALID_NAME_PHRASES:
        return False
    # Reject if it looks like a sentence (contains common sentence words)
    lower_words = set(name.lower().split())
    if lower_words & {'from', 'the', 'next', 'question', 'comes', 'please', 'go', 'ahead'}:
        return False
    # Reject if last word is a company suffix (e.g., "Sumangal Investments", "Kotak Securities")
    words = name.split()
    if len(words) >= 2 and words[-1].lower().rstrip('.') in _COMPANY_SUFFIXES:
        return False
    return True
...
def detect_qa_blocks(
    ...
    for pattern, pattern_name in QA_BLOCK_START_PATTERNS:
        for match in re.finditer(pattern, qa_section_text):
            ...
            # Reject captures that are not valid person names (garbage from greedy regex)
            if not _is_valid_questioner_name(questioner_name):
                logger.debug(
                    "qa_block_name_rejected",
                    name=questioner_name[:60],
