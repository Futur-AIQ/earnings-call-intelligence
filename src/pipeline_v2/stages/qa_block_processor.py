...
# Fallback: name alone on its own line (next non-blank lines are the content)
SPEAKER_ALONE_LINE_PATTERN = re.compile(
    r"^([A-Z][A-Za-z\.\-']+(?:\s+[A-Z][A-Za-z\.\-']+){0,3})\s*$",
    re.MULTILINE
)


def _extract_speaker_turns_primary(text: str) -> list[dict]:
    """Strategy 1: Strict 'Name: text' or 'Name - text' format."""
    lines = text.split('\n')
    turns = []
...
def _extract_speaker_turns_name_only_line(text: str) -> list[dict]:
    """Strategy 2: Name on its own line, followed by content lines."""
    lines = text.split('\n')
    turns = []
    current_speaker = None
    current_text = []
    ...
    for i, line in enumerate(lines):
        ...
        if not stripped:
            continue
        # Check if line is ONLY a speaker name (no content after it)
        if SPEAKER_ALONE_LINE_PATTERN.match(stripped):
            if current_speaker and current_text:
                turns.append({
                    "speaker": current_speaker,
                    "text": ' '.join(current_text),
    ...
    # Only return if we got meaningful turns (not just the header paragraph)
    return turns if len(turns) >= 2 else []


def _extract_speaker_turns_paragraphs(text: str) -> list[dict]:
    """Strategy 3: Split by blank lines. Each paragraph is a separate turn.

    Used when no speaker labels are found at all. The LLM will infer speakers
    from context.
    """
...
def _extract_speaker_turns(text: str) -> tuple[list[dict], str]:
    """Extract speaker turns using a cascade of fallback strategies.

    Returns (turns, extraction_method) where extraction_method describes
    which strategy succeeded.
    """
    # Strategy 1: Strict Name: text format
    turns = _extract_speaker_turns_primary(text)
    if turns:
        return turns, "name_colon_text"

    # Strategy 2: Name on its own line
    turns = _extract_speaker_turns_name_only_line(text)
    if turns:
        return turns, "name_only_line"

    # Strategy 3: Paragraph-based (no speaker labels at all)
    turns = _extract_speaker_turns_paragraphs(text)
    if turns:
        return turns, "paragraphs"

    # Strategy 4: Entire block as one unlabeled turn (last resort)
    full_text = text.strip()
    if full_text:
        return [{"speaker": "Unknown", "text": full_text, "line_num": 0}], "raw_block"
...
def _structure_qa_block_with_llm(
    ...
    extraction_method: str = "name_colon_text",
    ...
    Args:
        ...
        extraction_method: Which strategy was used to extract turns (affects LLM instructions)
    ...
    # Determine if speaker labels are available or if we're in fallback mode
    has_speaker_labels = extraction_method == "name_colon_text" or extraction_method == "name_only_line"
    unlabeled_note = ""
    if not has_speaker_labels:
        unlabeled_note = f"""
NOTE: Speaker labels were NOT detected in this block (extraction method: {extraction_method}).
The turns below are paragraphs or raw text WITHOUT explicit speaker labels.
Use the introduced questioner name and the speaker registry to INFER who is speaking:
- The introduced questioner ({block.questioner_name}) is the ANALYST asking questions
- Management speakers are those listed in the registry with role=management
- Turns that are questions belong to the analyst; turns that answer belong to management
"""
    ...
    for i, turn in enumerate(speaker_turns[:30]):
        ...
        speaker_display = speaker if speaker != "Unknown" else f"[Paragraph {i}]"
        turn_summary += f"[Turn {i}] {speaker_display}: {preview}\n"
...
1. VALIDATE: Is this a valid Q&A block?
   ...
   - Invalid: Opening remarks, moderator announcements only, no substantive Q&A
...
=== RULES ===
- Moderator/introduction turns should NOT be included in question/answer indices
...
- Even if speaker labels are missing, identify turns by content (questions vs. answers)
...
- Indices are 0-based turn numbers from the list above"""
    ...
    response = _invoke_llm(prompt, max_tokens=1500, require_json=True)
...
def extract_qa_from_blocks(
    ...
    for block in blocks:
        ...
        # Extract speaker turns from block — cascade of fallback strategies
        speaker_turns, extraction_method = _extract_speaker_turns(block.raw_text)
        ...
        result = _structure_qa_block_with_llm(block, speaker_turns, registry, extraction_method)
