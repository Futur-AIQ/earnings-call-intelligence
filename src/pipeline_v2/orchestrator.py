...
def run_pipeline_v2(
    ...
    try:
        ...
        if stage_callback:
            stage_callback("extraction", "running")
        state = _run_pdf_extraction(state, pdf_path)

        # Stage 0B: Text Cleaning (regex noise removal — part of extraction stage)
        state = _run_text_cleaning(state)
        if stage_callback:
            stage_callback("extraction", "completed")

        # Stage 1: Metadata Extraction
        if stage_callback:
            stage_callback("metadata", "running")
        state = _run_metadata_extraction(state)
        if stage_callback:
            stage_callback("metadata", "completed")

        # Stage 2: Boundary Detection
        if stage_callback:
            stage_callback("boundary", "running")
        state = _run_boundary_detection(state)
        if stage_callback:
            stage_callback("boundary", "completed")

        # Stage 3: Speaker Registry
        if stage_callback:
            stage_callback("speakers", "running")
        state = _run_speaker_registry(state)
        if stage_callback:
            stage_callback("speakers", "completed")

        # Stage 4: Q&A Extraction
        if stage_callback:
            stage_callback("qa", "running")
        if use_block_processor:
            state = _run_qa_block_extraction(state)
        else:
            state = _run_qa_extraction(state)
        if stage_callback:
            stage_callback("qa", "completed")

        # Stage 5: Strategic Extraction
        if stage_callback:
            stage_callback("strategic", "running")
        state = _run_strategic_extraction(state)
        if stage_callback:
            stage_callback("strategic", "completed")

        # Stage 6: Enrichment (optional, not tracked in UI)
