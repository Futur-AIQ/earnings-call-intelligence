...
/**
 * Process table cell children: convert literal <br> tags to line breaks
 * and handle bullet characters for readable multi-line cells.
 */
function processTableCellChildren(
  children: React.ReactNode,
  citeMap: Map<number, ChatCitation>,
  onCitationClick?: (citation: ChatCitation) => void
): React.ReactNode {
  return React.Children.map(children, (child) => {
    ...
    if (segments.length <= 1) {
      // No <br> tags — just run citation processing
      return processChildren([child], citeMap, onCitationClick);
    }

    // Multiple segments: join with real line breaks
    const parts: React.ReactNode[] = [];
      ...
      if (trimmed) {
        parts.push(
          <React.Fragment key={`seg-${i}`}>
            {processChildren([trimmed], citeMap, onCitationClick)}
          </React.Fragment>
        );
      }
    });
  ...
  return {
    ...
    table: ({ children, ...props }: React.HTMLAttributes<HTMLTableElement>) => (
      <div className="overflow-x-auto my-2">
        <table className="border-collapse border border-border text-sm w-full" {...props}>
          {children}
        </table>
      </div>
    ),
    ...
    th: ({ children, ...props }: React.HTMLAttributes<HTMLTableCellElement>) => (
      <th className="border border-border px-2 py-1.5 font-semibold text-left bg-muted/30" {...props}>
        {processTableCellChildren(children, citeMap, onCitationClick)}
      </th>
    ),
